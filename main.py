...
#===== 2
mat = mat[:,(per_gene_sum>=np.quantile(per_gene_sum, 0.05)) & (per_gene_sum<=np.quantile(per_gene_sum, 0.95))] #just remove extreme outliers
...
#===== 3
mat = mat[:,(CV>=np.quantile(CV, 0.05)) & (CV<=np.quantile(CV, 0.95))]


cells_expression = mat.sum(axis=1)

#===== 4
mat = mat[(cells_expression>=np.quantile(cells_expression, 0.05)) & (cells_expression<=np.quantile(cells_expression, 0.95)),:]
mat = np.log(mat+1)
